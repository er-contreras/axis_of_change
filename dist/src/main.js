(()=>{var k=document.createElement("template");k.innerHTML=`
  ...
  </header>`;var m=class extends HTMLElement{connectedCallback(){let e=k.content.cloneNode(!0);this.append(e)}};customElements.define("header-wrapper",m);var x=document.createElement("template");x.innerHTML=`
...
`;var f=class extends HTMLElement{connectedCallback(){this.append(x.content.cloneNode(!0)),"requestIdleCallback"in window?window.requestIdleCallback(()=>this.renderSVG()):setTimeout(()=>this.renderSVG(),100)}renderSVG(){let e=this.querySelector("svg"),t=100,n=90;e.innerHTML=`
    <defs>
  ...
  `,[{teeth:27,innerR:82,outerR:90,fill:"url(#steel-dark)",speed:45,dir:1},{teeth:30,innerR:66,outerR:66,fill:"white",speed:45,dir:1},{teeth:24,innerR:54,outerR:62,fill:"gray",speed:30,dir:-1},{teeth:16,innerR:42,outerR:50,fill:"#B02A2A",speed:30,dir:-1},{teeth:15,innerR:34,outerR:34,fill:"white",speed:20,dir:1},{teeth:15,innerR:28,outerR:28,fill:"gray",speed:20,dir:1},{teeth:15,innerR:20,outerR:20,fill:"white",speed:20,dir:1}].forEach(({teeth:i,innerR:h,outerR:l,fill:u,speed:p,dir:r})=>{let o=document.createElementNS("http://www.w3.org/2000/svg","path");o.setAttribute("d",this.calculatePrecisionGearPath(t,n,i,h,l)),o.setAttribute("fill",u),o.setAttribute("filter","url(#metal-gloss)"),o.style.filter="url(#metal-gloss) drop-shadow(1.5px 1.5px 2px rgba(0,0,0,0.6))";let c=document.createElementNS("http://www.w3.org/2000/svg","animateTransform");c.setAttribute("attributeName","transform"),c.setAttribute("type","rotate"),c.setAttribute("from",`${r===1?0:360} ${t} ${n}`),c.setAttribute("to",`${r===1?360:0} ${t} ${n}`),c.setAttribute("dur",`${p}s`),c.setAttribute("repeatCount","indefinite"),o.appendChild(c),e.appendChild(o)});let s=document.createElementNS("http://www.w3.org/2000/svg","circle");s.setAttribute("cx",t),s.setAttribute("cy",n),s.setAttribute("r","12"),s.setAttribute("fill","none"),s.setAttribute("stroke","#393939"),s.setAttribute("stroke-width","5"),e.appendChild(s)}calculatePrecisionGearPath(e,t,n,a,s){let i=[],h=Math.PI*2/n,l=h*.35,u=h*.25;for(let p=0;p<n;p++){let r=p*h;i.push(`${e+Math.cos(r-l)*a},${t+Math.sin(r-l)*a}`),i.push(`${e+Math.cos(r-u)*s},${t+Math.sin(r-u)*s}`),i.push(`${e+Math.cos(r+u)*s},${t+Math.sin(r+u)*s}`),i.push(`${e+Math.cos(r+l)*a},${t+Math.sin(r+l)*a}`);let o=r+h/2;i.push(`${e+Math.cos(o-l)*a},${t+Math.sin(o-l)*a}`)}return"M"+i.join(" L")+" Z"}};customElements.define("main-wrapper",f);var C=document.createElement("template");C.innerHTML=`
...
`;var y=class extends HTMLElement{connectedCallback(){var n;this.append(A.content.cloneNode(!0));let e=this.querySelector(".close-btn");(n=document.querySelector(".menu-button"))==null||n.addEventListener("click",()=>{this.style.display="block"}),e.addEventListener("click",()=>{this.style.display="none"}),this.querySelectorAll(".link").forEach(a=>{a.addEventListener("click",()=>{this.style.display="none"})})}};customElements.define("menu-wrapper",y);})();
