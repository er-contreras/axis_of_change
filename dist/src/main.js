(()=>{var k=document.createElement("template");k.innerHTML=`
  ...
  </header>`;var m=class extends HTMLElement{connectedCallback(){let e=k.content.cloneNode(!0);this.append(e)}};customElements.define("header-wrapper",m);var S=document.createElement("template");S.innerHTML=`
...
`;var f=class extends HTMLElement{connectedCallback(){this.append(S.content.cloneNode(!0)),this.renderSVG()}renderSVG(){let e=this.querySelector("svg"),t=100,n=90;e.innerHTML=`
    <defs>
  ...
  `,[{teeth:27,innerR:82,outerR:90,fill:"url(#steel-dark)",speed:45,dir:1},{teeth:30,innerR:66,outerR:66,fill:"white",speed:45,dir:1},{teeth:24,innerR:54,outerR:62,fill:"gray",speed:30,dir:-1},{teeth:16,innerR:42,outerR:50,fill:"#B02A2A",speed:30,dir:-1},{teeth:15,innerR:34,outerR:34,fill:"white",speed:20,dir:1},{teeth:15,innerR:28,outerR:28,fill:"gray",speed:20,dir:1},{teeth:15,innerR:20,outerR:20,fill:"white",speed:20,dir:1}].forEach(({teeth:s,innerR:h,outerR:l,fill:u,speed:p,dir:r})=>{let o=document.createElementNS("http://www.w3.org/2000/svg","path");o.setAttribute("d",this.calculatePrecisionGearPath(t,n,s,h,l)),o.setAttribute("fill",u),o.setAttribute("filter","url(#metal-gloss)"),o.style.filter="url(#metal-gloss) drop-shadow(1.5px 1.5px 2px rgba(0,0,0,0.6))";let c=document.createElementNS("http://www.w3.org/2000/svg","animateTransform");c.setAttribute("attributeName","transform"),c.setAttribute("type","rotate"),c.setAttribute("from",`${r===1?0:360} ${t} ${n}`),c.setAttribute("to",`${r===1?360:0} ${t} ${n}`),c.setAttribute("dur",`${p}s`),c.setAttribute("repeatCount","indefinite"),o.appendChild(c),e.appendChild(o)});let i=document.createElementNS("http://www.w3.org/2000/svg","circle");i.setAttribute("cx",t),i.setAttribute("cy",n),i.setAttribute("r","12"),i.setAttribute("fill","none"),i.setAttribute("stroke","#393939"),i.setAttribute("stroke-width","5"),e.appendChild(i)}calculatePrecisionGearPath(e,t,n,a,i){let s=[],h=Math.PI*2/n,l=h*.35,u=h*.25;for(let p=0;p<n;p++){let r=p*h;s.push(`${e+Math.cos(r-l)*a},${t+Math.sin(r-l)*a}`),s.push(`${e+Math.cos(r-u)*i},${t+Math.sin(r-u)*i}`),s.push(`${e+Math.cos(r+u)*i},${t+Math.sin(r+u)*i}`),s.push(`${e+Math.cos(r+l)*a},${t+Math.sin(r+l)*a}`);let o=r+h/2;s.push(`${e+Math.cos(o-l)*a},${t+Math.sin(o-l)*a}`)}return"M"+s.join(" L")+" Z"}};customElements.define("main-wrapper",f);var C=document.createElement("template");C.innerHTML=`
...
`,v=class extends HTMLElement{connectedCallback(){this.innerHTML=this.renderPrices()}renderPrices(){return`
    ...
    `}};customElements.define("pricing-wrapper",v);var A=document.createElement("template");A.innerHTML=`
...
`;var y=class extends HTMLElement{connectedCallback(){this.append(E.content.cloneNode(!0)),this.renderYear(),this.renderLists(),this.renderSocialLinks()}renderYear(){this.querySelector(".year").textContent=new Date().getFullYear()}renderLists(){let e=["Backend Architecture","System Performance Tuning","Infrastructure Automation","API Design & Scaling","CI/CD Pipeline Engineering"],t=["Language-Agnostic Mastery","Mission-Critical Reliability","Resource-Efficient Design","Seamless Deployment Workflows","Senior-Level Strategic Consulting"];this.fillList(".services-list",e),this.fillList(".reasons-list",t)}fillList(e,t){let n=this.querySelector(e);t.forEach(a=>{let i=document.createElement("li");i.textContent=a,n.appendChild(i)})}renderSocialLinks(){let e=[{name:"LinkedIn",href:"https://www.linkedin.com/in/er-contreras/",icon:"linkedin"},{name:"Twitter",href:"https://twitter.com/er_contreras_",icon:"twitter"},{name:"GitHub",href:"https://github.com/er-contreras",icon:"github"}],t=this.querySelector(".social-media");e.forEach(({name:n,href:a,icon:i})=>{let s=document.createElement("a");s.href=a,s.target="_blank",s.rel="noopener noreferrer",s.setAttribute("aria-label",n),s.innerHTML=x[i],t.appendChild(s)})}};customElements.define("footer-wrapper",y);var L=document.createElement("template");L.innerHTML=`
